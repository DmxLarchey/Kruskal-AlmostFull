...
#[global] Abbreviation Base := Prop (only parsing).

Module Base_Sums_Products.

  Abbreviation Absurd := False (only parsing).
  Abbreviation DepSum := ex (only parsing).
  Abbreviation NonDepSum := or (only parsing).
  Abbreviation NonDepProd := and (only parsing).
