...
these terms are computer generated by `Ltac` proof scripts inside this library.
...
In that setting grounded on the `Base` notation, the parametric definition of `af` becomes
```coq
...
- when `Base := Prop` then the formula `∃ₜ n, ∃ i j, i < j < n ∧ ...` means exactly `∃ n i j, i < j < n ∧ ...` 
